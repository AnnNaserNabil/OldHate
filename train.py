...
def run_kfold_training(config, comments, labels, tokenizer, device):
    ...
    with mlflow.start_run(run_name=f"{config.author_name}_batch{config.batch}_lr{config.lr}_epochs{config.epochs}"):
            ...
            for epoch in range(config.epochs):
                ...
                if val_metrics['macro_f1'] > best_macro_f1:
                    ...
                    if best_macro_f1 > best_overall_macro_f1:
                        ...
                        # Store the model object to save later
        ...
        if best_model_obj is not None:
            ...
            # Load the best state dict into the model object if it's not already there
            # (In our loop, best_model_obj points to the model that achieved the best overall F1)
            # However, since we are in a loop, the model object might have been modified in later epochs.
            # So we should re-load the best_fold_model state dict.
            best_model_obj.load_state_dict(best_fold_model)
            best_model_obj.save_pretrained(best_model_dir)
